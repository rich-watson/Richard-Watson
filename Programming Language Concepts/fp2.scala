...
object fp2 {
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    def f(b:String, a:String) : String = { b + a + term}
    xs match{
      case Nil => ""
  ...
  def joinTerminateRight (xs : List[String], delimiter : String) : String = {
    def f(a:String, b:String) : String = {delimiter + a + b}
    xs match {
      case Nil => ""
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    xs.indexOf(firstAux(a, xs))
    }
  def firstAux(a : Int, xs : List[Int]) : Int = {
    xs match {
      case Nil => throw new RuntimeException
      case y::ys => if (y < a) firstAux(a, ys) else y
    }
  }
